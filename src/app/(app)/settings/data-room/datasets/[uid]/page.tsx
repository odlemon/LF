        ...
        <div className="bg-surface border border-border/60 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row gap-6 justify-between items-start">
          ...
          <div className="flex gap-2 shrink-0 self-end md:self-start">
            <Button variant="secondary" onClick={() => setIsBatchOpen(true)}>
              <HiClipboardCopy className="w-4 h-4" /> Batch Upload
            </Button>
          </div>
        </div>
      )}
                ...
                <tbody className="divide-y divide-gray-100 font-semibold text-ink/80">
                  {isDocsLoading && documents.length === 0 ? (
                    ...
                    documents.map((doc) => {
                      ...
                      const needsMapping = doc.columnMappingStatus === "UNMAPPED" || doc.columnMappingStatus === "PARTIAL";
                              ...
                              {confirmDeleteUid === doc.uid ? (
                                ...
                                <>
                                  {isFailed && (
                                    <>
                                      <button
                                        ...
                                        className="p-1.5 hover:bg-canvas text-ink/60 hover:text-primary rounded-lg cursor-pointer"
                                      >
                                  ...
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 hover:bg-canvas rounded text-[10px] font-bold text-ink/60 hover:text-ink transition-colors"
                                  >
                                    Log
                                  </button>
                                  <button
                                    onClick={() => setConfirmDeleteUid(doc.uid)}
                                    className="p-1.5 hover:bg-red-50 text-ink/60 hover:text-red-700 rounded-lg transition-colors cursor-pointer"
                                  >
