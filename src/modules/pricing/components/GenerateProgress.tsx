    ...
    <div className="relative flex-1 flex flex-col min-h-0 overflow-hidden bg-canvas">
      ...
      <div className="relative flex-1 flex flex-col min-h-0 max-w-xl w-full mx-auto px-6 py-8 sm:py-10">
        <header className="shrink-0 mb-7">
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-ink/40 mb-3">
          ...
          <p className="mt-2 text-sm text-ink/50 leading-relaxed max-w-md">
              ...
              <li key={step.id} className="min-w-0">
                ...
                <p
                  className={`mt-2 text-[10px] font-semibold leading-tight truncate transition-colors ${
                    current ? "text-ink" : done ? "text-ink/50" : "text-ink/30"
                  }`}
                >
                  {step.label.split(" ")[0]}
                </p>
          ...
          <div className="relative px-4 py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-ink/45 leading-relaxed line-clamp-2">
                  {running
                    ? thinkingForTool(running.toolName).detail
                    : currentStage?.detail}
                </p>
              </div>
              <span className="text-xs tabular-nums text-ink/40 shrink-0 pt-0.5">
            ...
            <div className="mt-2.5 flex items-center gap-2 text-[11px] text-ink/40">
        ...
        <div className="flex-1 min-h-0 flex flex-col rounded-2xl border border-border/70 bg-surface overflow-hidden">
          ...
          {toolSteps.length === 0 ? (
            <div className="flex-1 flex items-center justify-center px-4 py-8 text-sm text-ink/35">
              Starting…
            </div>
          ) : (
            <ul
                    ...
                    <span
                      className={`relative flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${
                        step.status === "success"
                          ...
                          : step.status === "failed"
                            ? "border border-ink/25 text-ink/50"
