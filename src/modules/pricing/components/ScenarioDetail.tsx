  ...
  if (!scenario) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-10 text-center bg-surface">
        <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ink/30">
          Detail
        </p>
        <p className="mt-2 text-sm text-ink/40 max-w-[14rem] leading-relaxed">
      ...
      <div className="relative shrink-0 px-7 pt-7 pb-5 border-b border-border/50">
        ...
        {scenario.generationMode && scenario.generationMode !== "AGENT" && (
          <div className="mt-5 rounded-xl border border-border bg-field px-4 py-3 flex items-start gap-2.5">
            <HiOutlineExclamation className="w-4 h-4 text-ink/40 shrink-0 mt-0.5" />
            <div className="min-w-0">
              <p className="text-sm font-medium text-ink/80">
                Priced from your scope and rate card without the AI agent.
              </p>
              {scenario.generationNote && (
                <p className="mt-0.5 text-[12px] text-ink/45 leading-relaxed">
                  {scenario.generationNote}
                </p>
              )}
            </div>
        ...
        {scenario.aiReasoning && (
          <p className="mt-5 text-[13px] leading-relaxed text-ink/50 border-l-2 border-ink/15 pl-3">
            {scenario.aiReasoning}
          </p>
        )}
      </div>
        ...
        {editable ? (
          ...
          <>
            ...
            <div>
              ...
              {lines.length === 0 ? (
                <p className="text-sm text-ink/40">No line items</p>
              ) : (
                    ...
                    return (
                      <div key={phase}>
                        <div className="flex items-baseline justify-between gap-2 mb-2.5">
                          ...
                          <p className="text-xs tabular-nums font-semibold text-ink/55">
                          ...
                          {phaseLines.map((line) => (
                            ...
                            >
                              <div className="min-w-0">
                                ...
                                <p className="text-[11px] text-ink/40 mt-0.5 tabular-nums">
