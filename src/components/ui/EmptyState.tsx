...
export function EmptyState({ message, title, description, icon }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-6 bg-field/20 border border-dashed border-border/80 rounded-2xl text-center">
      <div className="w-12 h-12 rounded-xl bg-field border border-border text-ink/40 flex items-center justify-center mb-3">
      ...
      {displayDescription && (
        <p className="text-sm font-medium text-ink/55 max-w-sm leading-relaxed mt-1.5">
          {displayDescription}
        </p>
      )}
    </div>
